...
async function getTotalOwed(user) {
	let ans = await BlockchainSplitwise.getTotalOwed(user);
...
async function getLastActive(user) {
	...
	return lastActive > 0 ? lastActive : null;
}
...
async function add_IOU(creditor, amount) {
	...
	const tx = await BlockchainSplitwise.addIOU(defaultAccount, creditor, amount);
	await tx.wait();
}
